...
import { Registry, RegistryCorrupt, SETTINGS, planHandoffDescendants } from "./registry";
...
describe("Registry.load", () => {
  it("starts empty when the file is missing", () => {
    const reg = Registry.loadUnlocked(tmpFile());
  ...
  it("recovers from a corrupt JSON file by quarantining it", () => {
    ...
    const reg = Registry.loadUnlocked(file);
    expect(reg.projects).toEqual([]);
  ...
  it("opens only under a token covering the file's directory or an ancestor of it", () => {
    const file = tmpFile();
    const dir = path.dirname(file);
    expect(Registry.load(file, { dataRoot: dir, incarnation: 1 }, "quarantine").projects).toEqual([]);
    expect(
      Registry.load(file, { dataRoot: path.dirname(dir), incarnation: 1 }, "quarantine").projects,
    ).toEqual([]);
    const other = tmpFile();
    expect(() => Registry.load(file, { dataRoot: path.dirname(other), incarnation: 1 }, "quarantine")).toThrow(
      `authority token does not cover ${file}`,
    );
    expect(() =>
      Registry.load(file, { dataRoot: path.join(dir, "worktrees"), incarnation: 1 }, "quarantine"),
    ).toThrow("authority token does not cover");
  });

  it("under the stop policy refuses a corrupt file and leaves it in place", () => {
    const file = tmpFile();
    fs.writeFileSync(file, "{not json");
    expect(() => Registry.load(file, { dataRoot: path.dirname(file), incarnation: 1 }, "stop")).toThrow(
      RegistryCorrupt,
    );
  ...
  it("carries a worktree's resumeUnavailable flag through a round trip and never writes an unset one", () => {
    const file = tmpFile();
    const reg = Registry.loadUnlocked(file);
    reg.addSession(
      sessionRecord({ tabId: "stuck", worktree: { path: "/wt/a", branch: "a", base: "main", resumeUnavailable: true } }),
    );
    reg.addSession(sessionRecord({ tabId: "fine", worktree: { path: "/wt/b", branch: "b", base: "main" } }));
    const reloaded = Registry.loadUnlocked(file);
    expect(reloaded.sessions.find((s) => s.tabId === "stuck")?.worktree?.resumeUnavailable).toBe(true);
    expect("resumeUnavailable" in reloaded.sessions.find((s) => s.tabId === "fine")!.worktree!).toBe(false);
    // A non-boolean flag is a malformed element: the record is dropped, not the file.
    const raw = JSON.parse(fs.readFileSync(file, "utf8"));
    raw.sessions.find((s: { tabId: string }) => s.tabId === "stuck").worktree.resumeUnavailable = "yes";
    fs.writeFileSync(file, JSON.stringify(raw));
    expect(Registry.loadUnlocked(file).sessions.map((s) => s.tabId)).toEqual(["fine"]);
  });
  ...
  it("migrates a registry without defaultAgentMode to Plan", () => {
    ...
    expect(Registry.loadUnlocked(file).getSetting("defaultAgentMode")).toBe("plan");
  });
  ...
  it("accepts only bounded integer remote ports", () => {
    for (const [value, expected] of [
      ...
      expect(Registry.loadUnlocked(file).getSetting("remotePort")).toBe(expected);
    }
  });
});
...
describe("Registry persistence", () => {
  ...
  it("defaults dismissedAppUpdateVersion to null when the field is absent", () => {
    ...
    expect(Registry.loadUnlocked(file).getSetting("dismissedAppUpdateVersion")).toBeNull();
  });
  ...
  it("round-trips the dismissed app-update version across a reload", () => {
    ...
    expect(Registry.loadUnlocked(file).getSetting("dismissedAppUpdateVersion")).toBe("1.2.0");
  ...
  it("defaults dismissedOmpUpdateVersion to null when the field is absent", () => {
    ...
    expect(Registry.loadUnlocked(file).getSetting("dismissedOmpUpdateVersion")).toBeNull();
  });
  ...
  it("round-trips the dismissed omp-update version across a reload", () => {
    ...
    expect(Registry.loadUnlocked(file).getSetting("dismissedOmpUpdateVersion")).toBe("1.2.0");
  ...
  it("round-trips the plan format and falls back to html for anything unknown", () => {
    ...
    expect(Registry.loadUnlocked(file).getSetting("planFormat")).toBe("md");
    reg.setSetting("planFormat", "html");
    expect(Registry.loadUnlocked(file).getSetting("planFormat")).toBe("html");

    const junk = tmpFile();
    fs.writeFileSync(junk, JSON.stringify({ schemaVersion: 1, settings: { planFormat: "pdf" } }));
    expect(Registry.loadUnlocked(junk).getSetting("planFormat")).toBe("html");
    const absent = tmpFile();
    fs.writeFileSync(absent, JSON.stringify({ schemaVersion: 1, settings: {} }));
    expect(Registry.loadUnlocked(absent).getSetting("planFormat")).toBe("html");
  });
  ...
  it("round-trips the hibernate idle window and falls back to 30 for anything unknown", () => {
    ...
    expect(Registry.loadUnlocked(file).getSetting("hibernateIdleMinutes")).toBe(0);
    reg.setSetting("hibernateIdleMinutes", 1440);
    expect(Registry.loadUnlocked(file).getSetting("hibernateIdleMinutes")).toBe(1440);
    reg.setSetting("hibernateIdleMinutes", 30);
    expect(Registry.loadUnlocked(file).getSetting("hibernateIdleMinutes")).toBe(30);
    ...
    for (const junk of [-1, 30.5, 1441, "30", null, true]) {
      ...
      expect(Registry.loadUnlocked(f).getSetting("hibernateIdleMinutes")).toBe(30);
    }
    const absent = tmpFile();
    fs.writeFileSync(absent, JSON.stringify({ schemaVersion: 1, settings: {} }));
    expect(Registry.loadUnlocked(absent).getSetting("hibernateIdleMinutes")).toBe(30);
  });
  ...
  it("round-trips the stall watchdog window and falls back to 180 for anything unknown", () => {
    ...
    expect(Registry.loadUnlocked(file).getSetting("streamStallAbortSeconds")).toBe(0);
    reg.setSetting("streamStallAbortSeconds", 600);
    expect(Registry.loadUnlocked(file).getSetting("streamStallAbortSeconds")).toBe(600);
    ...
    for (const junk of [-1, 180.5, 3601, "180", null, true]) {
      ...
      expect(Registry.loadUnlocked(f).getSetting("streamStallAbortSeconds")).toBe(180);
    }
    const absent = tmpFile();
    fs.writeFileSync(absent, JSON.stringify({ schemaVersion: 1, settings: {} }));
    expect(Registry.loadUnlocked(absent).getSetting("streamStallAbortSeconds")).toBe(180);
  });
  ...
  it("round-trips advisor auto-reply and defaults to on for anything unknown", () => {
    ...
    expect(Registry.loadUnlocked(file).getSetting("advisorAutoReply")).toBe(false);
    reg.setSetting("advisorAutoReply", true);
    expect(Registry.loadUnlocked(file).getSetting("advisorAutoReply")).toBe(true);

    const junk = tmpFile();
    fs.writeFileSync(junk, JSON.stringify({ schemaVersion: 1, settings: { advisorAutoReply: "no" } }));
    expect(Registry.loadUnlocked(junk).getSetting("advisorAutoReply")).toBe(true);
    const absent = tmpFile();
    fs.writeFileSync(absent, JSON.stringify({ schemaVersion: 1, settings: {} }));
    expect(Registry.loadUnlocked(absent).getSetting("advisorAutoReply")).toBe(true);
  });
  ...
  it("round-trips stall auto-continue and defaults to on for anything unknown", () => {
    ...
    expect(Registry.loadUnlocked(file).getSetting("stallAutoContinue")).toBe(false);
    reg.setSetting("stallAutoContinue", true);
    expect(Registry.loadUnlocked(file).getSetting("stallAutoContinue")).toBe(true);
    ...
    for (const junk of ["no", 1, null, {}]) {
      ...
      expect(Registry.loadUnlocked(f).getSetting("stallAutoContinue")).toBe(true);
    }
    const absent = tmpFile();
    fs.writeFileSync(absent, JSON.stringify({ schemaVersion: 1, settings: {} }));
    expect(Registry.loadUnlocked(absent).getSetting("stallAutoContinue")).toBe(true);
  });
  ...
  it("round-trips desktop notifications and defaults to on for anything unknown", () => {
    ...
    expect(Registry.loadUnlocked(file).getSetting("desktopNotifications")).toBe(false);
    reg.setSetting("desktopNotifications", true);
    expect(Registry.loadUnlocked(file).getSetting("desktopNotifications")).toBe(true);
    ...
    for (const junk of ["yes", 0, null, {}]) {
      ...
      expect(Registry.loadUnlocked(f).getSetting("desktopNotifications")).toBe(true);
    }
    const absent = tmpFile();
    fs.writeFileSync(absent, JSON.stringify({ schemaVersion: 1, settings: {} }));
    expect(Registry.loadUnlocked(absent).getSetting("desktopNotifications")).toBe(true);
  });
  ...
  it("round-trips default advisor and falls back to off for anything unknown", () => {
    ...
    expect(Registry.loadUnlocked(file).getSetting("defaultAdvisor")).toBe(true);
    reg.setSetting("defaultAdvisor", false);
    expect(Registry.loadUnlocked(file).getSetting("defaultAdvisor")).toBe(false);

    const junk = tmpFile();
    fs.writeFileSync(junk, JSON.stringify({ schemaVersion: 1, settings: { defaultAdvisor: "no" } }));
    expect(Registry.loadUnlocked(junk).getSetting("defaultAdvisor")).toBe(false);
    const absent = tmpFile();
    fs.writeFileSync(absent, JSON.stringify({ schemaVersion: 1, settings: {} }));
    expect(Registry.loadUnlocked(absent).getSetting("defaultAdvisor")).toBe(false);
  });
...
describe("Registry mutations", () => {
  ...
  it("moveProject persists the new order across a reload", () => {
    ...
    reg.addProject("/abs/c");
    reg.moveProject("/abs/c", "/abs/a");
    expect(Registry.loadUnlocked(file).projects.map((p) => p.path)).toEqual(["/abs/c", "/abs/a", "/abs/b"]);
  });
  ...
  it("setSessionAdvisor survives a reload", () => {
    ...
    expect(Registry.loadUnlocked(file).sessions[0]).toMatchObject({ advisor: true, advisorModel: "a/b" });
  });
  ...
  it("remembers the complete model and advisor tuples per project", () => {
    ...
    expect(Registry.loadUnlocked(file).projects[0]).toMatchObject({
  ...
  it("setProjectDefaultModel pins, clears, and skips no-op writes", () => {
    ...
    expect(Registry.loadUnlocked(file).projects[0]).toMatchObject({ defaultModel: "p/m" });
  ...
  it("setProjectDefaultAdvisorModel pins, clears, and skips no-op writes", () => {
    ...
    expect(Registry.loadUnlocked(file).projects[0]).toMatchObject({ defaultAdvisorModel: "p/m:high" });
    const before = fs.readFileSync(file, "utf8");
    reg.setProjectDefaultAdvisorModel("/abs/proj", "p/m:high");
    expect(fs.readFileSync(file, "utf8")).toBe(before);
    reg.setProjectDefaultAdvisorModel("/abs/proj", null);
    expect(Registry.loadUnlocked(file).projects[0]).toMatchObject({ defaultAdvisorModel: null });
  });
  ...
  it("leaves session, project, and settings memory unchanged after a failed write and retries", () => {
    ...
    expect(Registry.loadUnlocked(file).sessions[0]).toMatchObject({
...
describe("Session sidebar order (#274)", () => {
  ...
  it("moveSession persists the new order across a reload", () => {
    ...
    expect(Registry.loadUnlocked(file).sessions.map((s) => s.tabId)).toEqual(["s-1", "s-3", "s-2"]);
  });
  ...
  it("seeds the frozen order from recency once and never re-sorts again", () => {
    ...
    expect(Registry.loadUnlocked(file).sessions.map((s) => s.tabId)).toEqual(["old", "new"]);
  });
...
describe("Registry worktree field", () => {
  it("round-trips a worktree record through save and load", () => {
    ...
    Registry.loadUnlocked(file).addSession(sessionRecord({ worktree }));
    const reloaded = Registry.loadUnlocked(file);
    expect(reloaded.sessions).toHaveLength(1);
  ...
  it("normalizes a record without a worktree to null on load", () => {
    const file = tmpFile();
    Registry.loadUnlocked(file).addSession(sessionRecord());
    const reloaded = Registry.loadUnlocked(file);
    expect(reloaded.sessions).toHaveLength(1);
