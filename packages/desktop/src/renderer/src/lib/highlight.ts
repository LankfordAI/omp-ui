 ...
 *
 * The shiki core, its grammar table, and the one-shot `tokenizeCode` live in
 * `@omp-ui/plan-doc` (shared with the plan pipeline and the headless
 * verifier); this module adds what only the live transcript needs — the
 * incremental streaming state machine and the React hooks — on top of that
 * single core. Until the highlighter resolves the caller renders plain text:
 * highlighting is an enhancement, never a gate on seeing the code.
 */
export { HIGHLIGHT_CHAR_CAP, langFromPath, resolveLang, tokenizeCode } from "@omp-ui/plan-doc";
