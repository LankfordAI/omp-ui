...
export function UpdatesPage() {
  ...
  return (
    <div className="space-y-3 px-4 py-3">
      {/* The host's own update comes first and reaches every client (#442 §10.2). */}
      <HostUpdateCard />

      {/* The omp-ui panel describes the desktop client's own artifact (#454): a browser client
          runs no artifact it could update, so it sees no such panel at all — printing
          "unversioned build" for a version it does not run would be a fake capability. */}
      {HAS_DESKTOP && (
