...
function fakeHost(opts: { instanceId?: string; favorites?: unknown } = {}): FakeHost {
  ...
  const table = {
    request: {
      [CH.getInstanceIdentity]: () => {
        ...
        return {
          instanceId: opts.instanceId ?? REMOTE_INSTANCE_ID,
    ...
    notify: {
      [CH.tabViewed]: (tabId: string | null) => record(CH.tabViewed, [tabId]),
...
const servers: HostServerHandle[] = [];
const rawServers: Server[] = [];

/** Token or password-session credential; the joined-instance header selects the role. */
...
async function serve(
  ...
  const handle = await startHostServer({
    ...
    hostVersion: REMOTE_VERSION,
    protocolRange: opts.protocolRange,
...
/**
 * A pre-#442 host: never answers the hello, has no instance:identity, answers every other
 * request with the old server's "unknown channel" rejection. Ignores credentials entirely.
 */
function serveProtocol1WithoutIdentity(): Promise<number> {
  const server = createServer();
  const wss = new WebSocketServer({ server, path: REMOTE_WS_PATH });
  wss.on("connection", (ws) => {
    ws.on("message", (raw: Buffer) => {
      const frame = JSON.parse(raw.toString("utf8")) as { t: string; id?: number; ch?: string };
      if (frame.t !== "req") return;
      ws.send(JSON.stringify({ t: "res", id: frame.id, ok: false, message: `unknown channel ${frame.ch}` }));
    });
  });
...
describe("joining", () => {
  ...
  it("dials as an instance-role protocol-2 client", async () => {
    const host = fakeHost();
    const server = await serve(host);
    const h = harness();
    ...
    expect(host.contexts[0]).toMatchObject({
      role: "instance",
      clientKind: "instance",
      clientVersion: "1.0.0",
      protocolVersion: HOST_PROTOCOL,
  ...
  it("marks a host whose verdict is incompatible as too old when its range ends below ours, never retrying", async () => {
    const host = fakeHost();
    const server = await serve(host, { protocolRange: { min: 1, max: HOST_PROTOCOL - 1 } });
    const h = harness();
    ...
    expect(h.timers.filter((t) => !t.cleared)).toEqual([]);
    // The refused socket never earned a table.
    expect(host.contexts).toEqual([]);
  });

  it("marks a host whose range starts above ours as too new", async () => {
    const host = fakeHost();
    const server = await serve(host, {
      protocolRange: { min: HOST_PROTOCOL + 1, max: HOST_PROTOCOL + 2 },
    });
    const h = harness();

    const summary = await join(h, server.port);
    expect(summary.status).toBe("incompatible");
    expect(summary.error).toContain("newer than this app");
    expect(h.timers.filter((t) => !t.cleared)).toEqual([]);
  });

  it("marks a protocol-1 host without instance:identity as incompatible and never retries", async () => {
    const port = await serveProtocol1WithoutIdentity();
    const h = harness();
...
describe("proxy and routing", () => {
  it("routes a tab-scoped request to the owning instance and keeps local tabs local", async () => {
    ...
    const routed = routeByTab(table, (id) => h.manager.ownerOf(id), h.manager, "conn-1");
  ...
  it("a closed connection clears its viewed tab at the owning instance", async () => {
    const host = fakeHost();
    const server = await serve(host);
    const h = harness();
    await join(h, server.port);
    ...
    viewed = host.nextNotify(CH.tabViewed);
    h.manager.connectionClosed("c1");
    expect(await viewed).toEqual([null]);

    // Nothing to clear a second time: a round trip proves no further notify rode the socket.
    h.manager.connectionClosed("c1");
    await h.manager.request(h.manager.summaries()[0]!.id, CH.getState, []);
    expect(host.notified.filter((n) => n.ch === CH.tabViewed)).toHaveLength(2);
  });
