...
export interface RemoteInstanceManagerDeps {
  ...
  localVersion: string;
  /** Mirrors a remote tab event into local sinks unchanged (HostApplication.send). */
  send: (channel: string, args: unknown[]) => void;
  /** Rebuilds and fans out BackendState (HostApplication.broadcast). */
...
const REMOTE_TOO_NEW = "remote omp-ui is newer than this app and cannot be joined";
...
export class RemoteInstanceManager {
  ...
  /** Which instance each connection last viewed a tab of, so a switch away can clear it there. */
  ...
  /**
   * Mirrors a connection's viewed-tab report to the owning instance (hibernation bookkeeping,
   * issue #271): the instance it moved away from hears `null`, the one it moved to hears the tab.
   * The peer keys the report by our socket to it — one connection per joined instance — so only
   * the tab rides the notify. `tab:viewed` is main's own to forward — it is not on the
   * renderer-facing proxy allowlist.
   */
  forwardViewed(connId: string, tabId: string | null): void {
    const next = tabId === null ? null : this.ownerOf(tabId);
    const previous = this.#lastInstanceByClient.get(connId) ?? null;
    if (previous !== null && previous !== next) {
      this.#joined(previous)?.notify("tab:viewed", [null]);
    }
    if (next !== null) this.#joined(next)?.notify("tab:viewed", [tabId]);
    if (next === null) this.#lastInstanceByClient.delete(connId);
    else this.#lastInstanceByClient.set(connId, next);
  }

  /** A connection is gone: whichever instance it last viewed a tab of hears `null`. */
  connectionClosed(connId: string): void {
    this.forwardViewed(connId, null);
  }
  ...
  async #dial(entry: Entry): Promise<void> {
    ...
    try {
      client = await this.#connect(entry.record.url, credential, {
        hello: {
          clientRole: "instance",
          clientKind: "instance",
          clientVersion: this.deps.localVersion,
          clientProtocol: HOST_PROTOCOL,
        },
      });
    } catch (err) {
      if (!this.#current(entry, generation)) return;
      if (err instanceof InstanceConnectError) {
        const { failure } = err;
        ...
        if (failure.kind === "incompatible") {
          // The host's own verdict (issue #442): its range tells which side is behind.
          this.#set(entry, {
            status: "incompatible",
            error: failure.protocolRange.min > HOST_PROTOCOL ? REMOTE_TOO_NEW : REMOTE_TOO_OLD,
          });
          return;
        }
      }
    ...
    try {
      ...
      if (entry.client !== client) return;
      this.#detach(entry);
      // A host that neither answers a hello nor identifies itself predates joining altogether.
      if (client.hello === null) this.#set(entry, { status: "incompatible", error: REMOTE_TOO_OLD });
      else this.#lost(entry, err instanceof Error ? err.message : String(err));
      return;
    }
    if (entry.client !== client) return;
