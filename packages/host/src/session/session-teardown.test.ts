...
import { CH, RpcClient, spawnOmp, watchLineageDir, type LineageEvent, type PtyHandle } from "@omp-ui/core";
 ...
 * fake omp binary so resolveOmpBinary succeeds at host construction.
 */
function setup(): BoundConnection {
  ...
  it(
    ...
    async () => {
      ...
      try {
        ...
        predecessor.exit(23);
        expect(ipc.sent).toContainEqual({ channel: CH.onPtyExit, args: [tabId, 23] });
        expect(ipc.sent).not.toContainEqual({ channel: CH.onPtyExit, args: [tabId, -1] });
      } finally {
        vi.useRealTimers();
      }
    },
...
describe("terminate escalation (issue #182)", () => {
  ...
  it("still reports the exit after an escalated reap (no suppressExit)", async () => {
    ...
    try {
      ...
      expect(ipc.sent).toContainEqual({ channel: CH.onPtyExit, args: [tabId, 137] });
    } finally {
      vi.useRealTimers();
    }
  });
