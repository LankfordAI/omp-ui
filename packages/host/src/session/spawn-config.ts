 ...
 *
 * `token` is the resume seam's proof (issue #442 §10.1; #450) that the caller
 * owns the data root the session's lineage lives under: a resume without a
 * claimed authority is a type error, and a token with no root is a forgery.
 * A worktree the migration could not re-link (`resumeUnavailable`) is refused
 * outright — omp would start in a directory git no longer knows as a checkout.
 */
export async function prepareResumeRecord(
  ...
  deps: PrepareResumeDeps,
    ...
    if (token.dataRoot === "") throw new Error("resume requires an authority token naming its data root");
    if (record.worktree?.resumeUnavailable === true) {
      throw new Error(
        "the session's worktree checkout could not be repaired after relocation; release or re-create the worktree",
      );
    }
