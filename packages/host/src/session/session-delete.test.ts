...
import { CH, resolveSessionLocation, spawnOmp } from "@omp-ui/core";
...
beforeEach(() => {
  ...
  spawnOmpMock.mockImplementation((opts) => {
    ...
    return {
      id: opts.id,
      pid: 1000 + spawnedSignals.length,
      onData: () => vi.fn(),
      onExit: (cb) => {
        exitCb = cb;
      },
...
describe("session:delete", () => {
  it("erases the record, the transcript, its artifacts, and the archived copy", async () => {
    ...
    const broadcast = ipc.sent.filter((m) => m.channel === CH.onStateChanged).at(-1);
  ...
  it("stops a live session, then erases the record and its files", async () => {
    ...
    expect(ipc.sent.filter((m) => m.channel === CH.onPtyExit)).toEqual([]);
  });
