...
import type { BreadcrumbEntry, KeyCipher, PtyHandle } from "@omp-ui/core";
...
function setup(opts: { mode?: "pty" | "rpc-ui"; project?: string; attention?: AttentionSink; providerEnv?: Record<string, string>; hasOAuthProvider?: () => boolean; spawnGate?: SpawnGate; planVerify?: (html: string, themeId: string, signal: AbortSignal) => Promise<Core.PlanRenderResult>; ledger?: ChildrenLedger } = {}): {
  ...
  const registry = Core.Registry.loadUnlocked(registryFile);
...
describe("OS attention hooks (issue #271)", () => {
  ...
  it("viewed reports are keyed by connection and dropped when it closes", () => {
    vi.useFakeTimers();
    try {
      const { manager } = setup();
      manager.setViewedTab("conn-a", TAB);
      expect(manager.isViewed(TAB)).toBe(true);

      // Another connection's report does not clear the first one's.
      manager.setViewedTab("conn-b", "other");
      expect(manager.isViewed(TAB)).toBe(true);

      manager.connectionClosed("conn-a");
      expect(manager.isViewed(TAB)).toBe(false);
      expect(manager.isViewed("other")).toBe(true);

      // A report that goes silent stops protecting on its own.
      vi.advanceTimersByTime(15 * 60_000 + 1);
      expect(manager.isViewed("other")).toBe(false);
    } finally {
      vi.useRealTimers();
    }
  });
...
describe("worktree sessions (issue #224)", () => {
  ...
  it("refuses to resume a worktree session the migration could not re-link (resumeUnavailable)", async () => {
    const { manager, registry } = setup();
    const project = await gitProject(base);
    registry.addProject(project);
    const worktreePath = Core.mintWorktreePath(worktreesRoot(), project, "omp-ui/wt-stuck");
    fs.mkdirSync(worktreePath, { recursive: true });
    const record = registry.addSession(
      ownedSessionRecord({
        tabId: "tab-stuck",
        projectCwd: project,
        mode: "pty",
        worktree: { path: worktreePath, branch: "omp-ui/wt-stuck", base: null, resumeUnavailable: true },
      }),
    );

    await expect(
      manager.spawn({ origin: "resume", resumeTabId: record.tabId, cols: 80, rows: 24 }),
    ).rejects.toThrow("could not be repaired after relocation");
    expect(spawnOmpMock).not.toHaveBeenCalled();
  });
  ...
  it("spawns a fresh session into an existing checkout via the reuse arm (issue #316)", async () => {
    ...
    // Compare the directory to itself — git's textual path output is
    // platform-dependent in shape and case (issue #317); a second cut
    // would add a leaf next to the reused checkout.
    const projectDir = path.dirname(worktreePath);
...
describe("children ledger hooks (#450)", () => {
  interface FakeLedger {
    ledger: ChildrenLedger;
    added: Array<{ entry: ChildEntry; liveAtAdd: boolean }>;
    removed: number[];
  }

  /** Records what the manager reports, and whether the tab was already live when `add` ran. */
  function fakeLedger(isLive: () => boolean): FakeLedger {
    const added: FakeLedger["added"] = [];
    const removed: number[] = [];
    const ledger = {
      bootId: () => "boot-T",
      add: (entry: ChildEntry) => {
        added.push({ entry, liveAtAdd: isLive() });
      },
      remove: (pid: number) => {
        removed.push(pid);
      },
  ...
  it("ledgers a pty child before its tab is live and removes it on exit", async () => {
    const ref: { manager: SessionManager | null } = { manager: null };
    const fake = fakeLedger(() => ref.manager?.isLive(TAB) ?? false);
    const { manager } = setup({ ledger: fake.ledger });
    ref.manager = manager;
    await resume(manager);
    const pid = fakePtys[0]!.pid;
    expect(fake.added).toEqual([
      {
        liveAtAdd: false,
        entry: expect.objectContaining({
          pid,
          pgid: process.platform === "win32" ? 0 : pid,
          bootId: "boot-T",
          executable: "/test/omp",
          kind: "pty",
          tabId: TAB,
          lineageDir: expect.stringContaining(LINEAGE),
        }),
      },
    ]);
    expect(fake.added[0]!.entry.procStartMs).toBeGreaterThan(0);
    expect(manager.isLive(TAB)).toBe(true);
    fakePtys[0]!.exit(0);
    expect(fake.removed).toEqual([pid]);
    expect(manager.isLive(TAB)).toBe(false);
  });

  it("ledgers an rpc-ui child by pid alone (it shares the host's group) and removes it on exit", async () => {
    const ref: { manager: SessionManager | null } = { manager: null };
    const fake = fakeLedger(() => ref.manager?.isLive(TAB) ?? false);
    const { manager } = setup({ mode: "rpc-ui", ledger: fake.ledger });
    ref.manager = manager;
    await manager.spawn({ origin: "resume", resumeTabId: TAB, cols: 80, rows: 24, planMode: false });
    const pid = rpcInstances[0]!.pid;
    expect(fake.added).toEqual([
      { liveAtAdd: false, entry: expect.objectContaining({ pid, pgid: 0, kind: "rpc-ui", tabId: TAB }) },
    ]);
    rpcInstances[0]!.exit(0);
    expect(fake.removed).toEqual([pid]);
  });
