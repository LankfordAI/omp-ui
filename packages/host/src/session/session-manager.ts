...
import { NO_ATTENTION, type AttentionSink } from "./trackers/attention-tracker";
import {
  createPtyLiveEntry,
  createRpcLiveEntry,
  type LiveEntry,
...
import { HibernationTracker } from "./trackers/hibernation-tracker";
import { CapabilityControlTracker } from "./trackers/capability-control-tracker";
import { PlanGateTracker, type PlanGate } from "./trackers/plan-gate-tracker";
import { PlanPreflightController } from "../verifier/plan-preflight";
import { readConfinedPlanFile } from "../verifier/plan-file";
import { GoalStatusTracker } from "./trackers/goal-status-tracker";
import { prepareResumeRecord, writeRpcExtensions, writeRpcOverlays, writeSessionOverlays } from "./spawn-config";
import { STALL_CONTINUE_LEAD, StallContinueTracker } from "./trackers/stall-continue";
import { StallWatchdog } from "./trackers/stall-watchdog";
import { TurnTracker } from "./turns";
import { ViewTracker } from "./trackers/view-tracker";
...
export interface SessionManagerDependencies {
  registry: Registry;
  /** Proof the owner claimed the data root (issue #442 §10.1); the resume seam demands it. */
...
export class SessionManager {
  ...
  private readonly frameObservers: FrameObserver<LiveEntry>[] = [];
  private readonly hibernation: HibernationTracker;
  private readonly stallWatchdog: StallWatchdog;
  private readonly stallContinue: StallContinueTracker;
  ...
  constructor(private readonly deps: SessionManagerDependencies) {
    ...
    this.stallWatchdog = new StallWatchdog({
      ...
      awaitingHumanAnswer: (tabId) => this.awaitingHumanAnswer(tabId),
      onAbort: (tabId) => this.stallContinue.noteWatchdogAbort(tabId),
    });
    ...
    // The host-side stall auto-continue guard (issue #442). An auto-prompt
    // must not bypass a human gate, a goal's own loop, or the app switch.
    this.stallContinue = new StallContinueTracker({
      canContinue: (tabId) =>
        this.live.get(tabId)?.kind === "rpc-ui" &&
        !this.awaitingHumanAnswer(tabId) &&
        deps.registry.getSetting("stallAutoContinue") &&
        this.planGates.gate(tabId)?.pending == null &&
        !this.goals.ownsSession(tabId),
      dispatch: (tabId) =>
        this.rpcSend(tabId, {
          type: "prompt",
          id: `omp-ui-stall-${Date.now()}`,
          message: STALL_CONTINUE_LEAD,
          streamingBehavior: "followUp",
        }),
      notice: (tabId, text, level) =>
        deps.send(CH.onRpcFrame, tabId, {
          type: "omp_ui_notice",
          level,
          source: "omp-ui",
          message: text,
        }),
      attention: deps.attention ?? NO_ATTENTION,
    });
    this.frameObservers = [
      this.hibernation,
      this.planGates,
  ...
  /** Adds a per-concern observer after construction (the host's attention tracker). */
  registerFrameObserver(observer: FrameObserver<LiveEntry>): void {
    this.frameObservers.push(observer);
  }

  /** A terminal tab: its stream is a PTY, so no frame-derived state exists for it. */
  isPtyTab(tabId: string): boolean {
    return this.live.get(tabId)?.kind === "pty";
  }
  ...
  /**
   * Ledgers the child BEFORE its tab is published live (#450): a host that
   * dies between the two leaves a child the next authority can still find.
   * Durable on return — `ChildrenLedger.add` fsyncs.
   */
  private recordChild(entry: LiveEntry, executable: string, pgid: number): void {
    const ledger = this.deps.ledger;
    if (ledger === undefined) return;
    const pid = entry.kind === "pty" ? entry.pty.pid : entry.rpc?.pid;
    ...
    ledger.add({
      ...
      executable,
      kind: entry.kind,
      tabId: entry.record.tabId,
      lineageDir: path.join(this.deps.getSessionsRoot(), entry.record.lineageDir),
    });
  }
  ...
  private handleExit(tabId: string, entry: LiveEntry, exitCode: number): void {
    entry.markExited();
    const pid = entry.kind === "pty" ? entry.pty.pid : entry.rpc?.pid;
    if (pid !== undefined) this.deps.ledger?.remove(pid);
  ...
  private async spawnInner(req: SpawnRequest): Promise<{ tabId: string }> {
    ...
    try {
      if (req.origin === "resume") {
        ...
        record = await prepareResumeRecord(this.deps.authority, existing, {
  ...
  private async rollbackSpawn(
    ...
    if (childStopped) {
      ...
      if (watcherStopped) {
        ...
        this.stallContinue.dispose(tabId);
  ...
  private async spawnPty(
    ...
    // node-pty children lead their own process group (setsid), so the group
    // is the pid; ConPTY has no groups.
    this.recordChild(entry, ompPath, process.platform === "win32" ? 0 : ptyHandle.pid);
  ...
  private async spawnRpc(
    ...
    wireRpc(entry, rpc);
    // Piped children stay in the host's own group, so only the pid is
    // theirs to signal; a failed spawn has no pid and nothing to ledger.
    if (rpc.pid !== undefined) this.recordChild(entry, ompPath, 0);
    this.live.set(record.tabId, entry);
    this.watcherHub.start(record);
    await this.deps.broadcast();
    return { tabId: record.tabId };
  ...
  setViewedTab(connId: string, tabId: string | null): void {
    this.viewTracker.setViewedTab(connId, tabId);
  }

  /** The connection is gone: its viewed report stops protecting anything (issue #442). */
  connectionClosed(connId: string): void {
    this.viewTracker.connectionClosed(connId);
  }

  /** True while any connection's fresh viewed report names the tab (issue #266). */
  isViewed(tabId: string): boolean {
    return this.viewTracker.isViewed(tabId);
  }
