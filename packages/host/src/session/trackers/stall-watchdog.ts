...
export interface StallWatchdogDeps {
  ...
  /** The abort was sent: the coming turn end is a stall end for auto-continue (issue #442). */
 ...
 * backstop.
 */
export class StallWatchdog implements FrameObserver<LiveEntry> {
  /**
  ...
  private abortStalledTurn(
    ...
    this.deps.send(CH.onRpcFrame, tabId, {
      ...
      level: "warn",
      source: "omp-ui",
      // Machine-readable marker of the abort: the turn end it produces reads
      // "aborted", which the frame-side stall classifier can never place.
      reason: "stall-abort",
      message:
