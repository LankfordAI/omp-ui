 ...
 * snapshot standing.
 */
export class GoalStatusTracker implements FrameObserver<LiveEntry> {
   ...
   * has scheduled or is running. Bounded auto-prompts must never restart,
   * contradict, or race an unbounded goal loop.
   */
  ownsSession(tabId: string): boolean {
    const snapshot = this.latest.get(tabId);
    if (snapshot === undefined) return false;
    if (snapshot.continuation !== "idle") return true;
    return snapshot.goal !== null && snapshot.goal.status !== "complete";
  }
