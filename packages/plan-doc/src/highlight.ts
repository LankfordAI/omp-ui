...
/**
 * Syntax highlighting: the shared shiki core (issue #27).
 *
 ...
 *
 * The theme is built from a runtime theme's `code` palette (`themes.ts`)
 * rather than a stock shiki theme, so highlighted code sits on the same
 * planes as the rest of the transcript and follows a theme switch. The
 * signal accent is deliberately absent from every theme's `code` map
 * (ADR-0004): it means agent liveness, never syntax.
 *
 * This module owns the ONE highlighter core and its grammar/theme
 * bookkeeping; the renderer's streaming tokenizer and hooks
 * (`lib/highlight.ts` there) build on `getHighlighterCore` and `ensureLang`
 * rather than creating a second core.
 */
...
/**
 * Themes registered on the core, seeded with the one `getHighlighterCore`
 * builds it with. A theme is only paid for once the user actually switches
 * to it.
 */
...
/** The single lazily created highlighter core, shared by every tokenizer. */
...
export function ensureLang(core: HighlighterCore, lang: string): Promise<void> {
