...
Each joined instance is a collapsible group after the local projects, headed by its nickname, a status dot, and the connection URL. The group holds that instance's registered projects and their owned sessions exactly as the other app's own sidebar shows them. A project may share its absolute path with a local project; the two are separate groups.
...
Registry and session lifecycle actions run on the remote instance: registering, removing, and reordering projects; new, resume, terminate, restart, hibernate, fork, move, and delete; mode switches; model and advisor changes; worktree sessions and branch actions; plan review; the capabilities viewer and its MCP, skills, and tool controls; the console shell; typing, resizing, and pasting images. Hibernation policy also follows the remote's viewed-tab rule, so a remote session you are looking at is exempt on that host just as a local one is here.
