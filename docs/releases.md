...
2. `release-linux` runs on the org's self-hosted Linux pool (`[self-hosted, gfx1201]`): a fresh container per job under a single-job registration (issue #629), asserted by the lane's first step — a lane lands on a label no registered runner serves, it queues forever rather than failing (issue #631). It installs dependencies, runs the repository typecheck and tests, runs the Electron-runtime `node-pty` smoke, stamps the version, builds the AppImage, and requires exactly one AppImage plus a `latest-linux.yml` that names it and contains SHA-512 metadata. It then uploads the AppImage and Linux metadata into the draft.
