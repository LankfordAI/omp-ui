...
- Inline `code` chips in HTML plans rendered in **plan review** now ride a light canvas-family tint instead of the theme's dark code plane, keeping their surrounding ink so code inside a link stays link-coloured; block code keeps its raised plane (issue #380).
