...
3. `package-macos`, `release-windows`, and `fedora-no-display-gate` all depend on `release-linux`, so they start only after Linux succeeds and may run concurrently.
...
6. `fedora-no-display-gate` runs on the self-hosted Fedora runner with `DISPLAY` and `WAYLAND_DISPLAY` unset. It runs `npm run test:live -w @omp-ui/host` with any skipped test counted as failure — real child process groups, an authenticated loopback WebSocket, plan preflight in the packaged headless Chrome with no display — then packages and smoke-tests the host again and publishes `package-record-fedora-no-display-gate`. It is the evidence that a machine with no graphical session can run omp-ui.
