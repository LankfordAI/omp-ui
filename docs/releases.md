...
- 0.12.0 withdraws the persistent headless host introduced in 0.11.0 and 0.11.1 and returns to the single Electron application of 0.10.6; that work continues on the `experimental/persistent-host` branch ([#479](https://github.com/LankfordAI/omp-ui/issues/479)). The version is higher than 0.11.1 so a 0.11.x install picks this release up through its normal update check. A machine that ran 0.11.x has had its projects, sessions, remote instances, worktrees, and provider keys moved into the host's data root by the host's one-way migration; on such a machine run `omp-ui-host uninstall`, move `registry.json`, `remote-instances.json`, `oauth-login/`, `worktrees/`, and `logs/` from the host data root back into the app's user-data directory, run `git worktree repair` in each moved worktree, and enter provider keys again.
