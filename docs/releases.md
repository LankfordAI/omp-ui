...
- A first launch of the desktop shell opens the **Getting started** checklist: a state-driven overlay over the four gates of a fresh install — the managed `omp` binary, a provider credential, a registered project, a first owned session — each read live from backend state, with one working action per unmet step. It appears once, gated on the new `gettingStartedSeen` registry setting; a registry that already carries projects or sessions counts as seen, so an upgrade never pops, and remote renderers never auto-open. The command palette and the Welcome empty state re-open it ([#623](https://github.com/LankfordAI/omp-ui/issues/623)).
- **Add project** from the Getting started checklist and from the Welcome empty state lists directories again: the click event React passed as the first argument was being taken for the picker's instance id, so every browse queried a nonexistent remote instance and the picker sat on "No matching directories" forever ([#624](https://github.com/LankfordAI/omp-ui/issues/624)).
- A manual compaction that outruns the 30-second response budget no longer fires a failure banner while silently dropping the *context compacted* marker: the compact response frame is now the completion event, and plan execution waits out a slow boundary (a 15-minute deadline) instead of mistaking slow for stuck. The session HUD reads *compacting* from the renderer's own tab record ([#625](https://github.com/LankfordAI/omp-ui/issues/625)).
- Pull-request CI from forks runs on a GitHub-hosted runner: the job body moved into a reusable workflow with same-repo and fork callers, keeping unaudited fork code off the self-hosted pool that builds release artifacts ([#626](https://github.com/LankfordAI/omp-ui/issues/626)).
...
- Every self-hosted Linux job now opens with an isolation check — single-job registration, unused filesystem, no reachable Docker socket or registration credential — reported without gating. The lane cutover to a dedicated `omp-ui-linux` fresh-container pool queued indefinitely against a label no runner serves and was reverted to the `gfx1201` pool; the cutover waits for the pool itself ([#629](https://github.com/LankfordAI/omp-ui/issues/629), [#631](https://github.com/LankfordAI/omp-ui/issues/631)).
- The browser pane keeps its HiDPI pin after agent CDP sessions: a forwarded `Emulation` override or a clipped/surface-wide screenshot can clobber or drop the host's device-metrics override, and Chromium ignores a resend identical to the last one, so the host clears its own override before re-asserting and re-pins on every client detach ([#630](https://github.com/LankfordAI/omp-ui/issues/630)).
...
1. `prepare` runs on a GitHub-hosted runner with no checkout. It creates the GitHub release as a **draft** if it does not exist yet, with `--verify-tag`, a title equal to the version without `v`, and empty notes. A tag missing on the remote fails here and every lane is skipped. Every other job depends only on `prepare`, so the platform lanes start together once the draft exists.
2. `release-linux` runs on the org's self-hosted Linux pool (`[self-hosted, gfx1201]`). Its first step reports the isolation check without gating until issue #629's fresh-container pool ships (issue #631). It installs dependencies, runs the repository typecheck and tests, runs the Electron-runtime `node-pty` smoke, stamps the version, builds the AppImage, and requires exactly one AppImage plus a `latest-linux.yml` that names it and contains SHA-512 metadata. It then uploads the AppImage and Linux metadata into the draft.
3. `release-notes` checks out the full history, resolves the previous tag with `git describe --tags --match "v*" --abbrev=0 <tag>^` (the `v*` match keeps the rolling `nightly` tag of issue #480 out of the range math), lists the PRs merged between the two tags, groups commits by the issue or PR their subjects reference, lifts the tag's `## Unreleased` bullets into `## Highlights`, and writes the result into the draft. A failure of this job never blocks publishing: a published release with no notes beats a finished release stranded in draft, where the updater cannot see it.
4. `package-macos` expands to Apple Silicon and Intel packages, both built on `macos-15` Apple Silicon runners: nothing in the lane executes the app, and the Intel runner is about twice as slow at every step. Each job installs dependencies, runs the `node-pty` smoke, stamps the version, and packages one DMG and one ZIP from the shipped `node-pty` prebuild without rebuilding it. Packaging uses the Developer ID certificate from `CSC_LINK` and `CSC_KEY_PASSWORD`. The release command disables electron-builder's built-in one-attempt notarization. A retrying `notarytool` script then submits the DMG with the Apple credentials, staples the app and DMG, and rebuilds the ZIP around the stapled app. Verification requires one thin app and `prebuilds/darwin-<arch>/pty.node` for the requested architecture, a valid Developer ID signature from `APPLE_TEAM_ID`, Gatekeeper acceptance, and a valid app staple. Each job then uploads its DMG, ZIP, and `SHA256SUMS-macos-<arch>.txt` directly into the draft; the macOS Preview and Nightly workflows call the same reusable workflow without a `release_tag` and receive the files as a `macos-package-<arch>` artifact instead.
5. `test-windows` runs the core and desktop Windows test suites; `ci.yml` runs on Linux only — the self-hosted runner for same-repo events, a GitHub-hosted `ubuntu-latest` runner for fork pull requests (issue #626) — so these are the project's only Windows test coverage. `release-windows` runs beside it: it installs dependencies, runs the `node-pty` smoke against the `win32-x64` prebuild, stamps the version, and packages NSIS with certificate discovery disabled. No MSVC toolchain is installed and nothing is rebuilt. Verification requires the unpacked app, x64 native modules, and ConPTY support files. The job then requires exactly one installer, its matching blockmap, and a `latest.yml` that names the installer and contains SHA-512 metadata before uploading all three. It performs no Authenticode signing; this is the unsigned preview accepted by [ADR-0013](adr/0013-unsigned-windows-preview-packages.md).
6. `release-manifest` depends on successful Linux, macOS, Windows, and `test-windows` jobs, and waits for `release-notes` to finish without gating on its result. It downloads exactly one AppImage, two DMGs, two ZIPs, and one Windows installer. It rejects duplicate or missing names, writes the sorted six-line `SHA256SUMS.txt`, and uploads it. It then calculates SHA-512 and size for all four macOS files, writes `latest-mac.yml`, and uploads the feed. Before publishing it warns in the run log when the draft's body is still empty.
