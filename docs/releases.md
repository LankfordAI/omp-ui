...
- The native transcript takes a Transcript width step — Comfortable, Wide (the new default), or Full — that caps the transcript column and the composer card together, so their edges align and tool cards, code, and diffs use the wider column while prose keeps a readable measure ([#391](https://github.com/LankfordAI/omp-ui/issues/391)).
- Speaker labels hang: when the centred transcript column leaves enough slack on both sides, the "assistant" label sits in the left gutter and "you" in the right, top-aligned with their first row; narrower columns keep the stacked labels ([#392](https://github.com/LankfordAI/omp-ui/issues/392)).
- Glass chrome — a new Appearance step (Off / Subtle / Frosted) — lets the sidebar, inspector rail, title bar, composer card, sheets, and modals show a soft backdrop wash through them, decided with `backdrop-filter` so text is never blurred and terminals and the reading plane stay opaque ([#393](https://github.com/LankfordAI/omp-ui/issues/393), [ADR-0026](adr/0026-glass-chrome-via-backdrop-filter.md)).
- Settings → Providers gains **Preferred provider**, which chooses the provider omp's native `web_search` tool tries first by writing omp's own `providers.webSearchOrder` global value (`Automatic` clears it, so unlisted providers keep omp's fallback order). The choices come from the installed omp binary — probed from its own `omp search --provider` flag validation, never a list omp-ui maintains — and a value a focused project overrides is badged `project` instead of silently shown as the global choice ([#394](https://github.com/LankfordAI/omp-ui/issues/394), [ADR-0027](adr/0027-web-search-provider-list-discovered-from-omp.md)).
- The **Capabilities** button in the session HUD, the compact sheet, and the command palette now opens the viewer at global scope, and the project settings dialog gains **Skills** and **Tools** sections beside MCP servers and model pins ([#383](https://github.com/LankfordAI/omp-ui/issues/383), [ADR-0025](adr/0025-scoped-capability-catalogs.md)). Both views are a **capability catalog** — config truth from the settings layers omp publishes and the `SKILL.md` roots it discovers, never an imitation of a live session's roster and never a probe session, so opening a dialog connects to nothing. A global switch writes omp's own config and a project switch edits `.omp/config.yml` through a line-scoped editor that preserves every untouched byte, comments included, and refuses any shape omp's writer would not emit; unknown tool or gate names throw before a write. The session-pinned viewer keeps its rosters and session-local tool switches, reachable from the palette's "Capabilities for this session" and Settings → omp.
- **Plan review** renders its canvas in the active theme instead of a fixed light page: the guardrail canvas, ink, inline `code` chips, links, and diagram text derive from the theme's tokens, mermaid and hand-drawn SVG paint is re-fitted for a dark canvas, and the authoring prompt stops mandating light ([#384](https://github.com/LankfordAI/omp-ui/issues/384)).
- One **Finish worktree dialog** retires the inline merge UI on the worktree and branch chips: it states the facts — ahead/behind, conflicts predicted or listed, where the destination is checked out — then asks where the work goes and what happens to the session. The destination resolves from the recorded base, then the default branch, then the first branch that is not the session's, and a destination checked out elsewhere is merged in a throwaway scratch worktree that is swept afterwards ([#385](https://github.com/LankfordAI/omp-ui/issues/385)). Work can land as a merge commit or as a kept branch, renamed first so the merge subject names the final branch ([#386](https://github.com/LankfordAI/omp-ui/issues/386)); merge-back conflicts are previewed with `git merge-tree` and the destination can be synced into the worktree so they resolve in the session that owns them ([#387](https://github.com/LankfordAI/omp-ui/issues/387)); releasing a session now refuses to discard uncommitted work instead of force-removing the checkout ([#388](https://github.com/LankfordAI/omp-ui/issues/388)); placeholder branches take their name from the first prompt and rename themselves automatically unless you already renamed ([#389](https://github.com/LankfordAI/omp-ui/issues/389)); and a new worktree session can start on an existing local branch ([#390](https://github.com/LankfordAI/omp-ui/issues/390)).
- The desktop composer now floats over its transcript: the card and the pending extension dialog above it lift clear of the pane's bottom edge, the native transcript runs the full height of the session column beneath them, and the tab publishes the stack's measured height so the newest row always scrolls clear while older content passes behind the glass ([#395](https://github.com/LankfordAI/omp-ui/issues/395)). The card carries the reading plane's own tone rather than the raised plane, so the float reads as glass defined by its hairline, shadow, and blur instead of a gray band over the prose ([#396](https://github.com/LankfordAI/omp-ui/issues/396)), and the bottom tail fade now anchors to the pane's edge and ends in a constant band, so the whole floating stack sits on one tone ([#398](https://github.com/LankfordAI/omp-ui/issues/398)).
- `readDefaultBranch` no longer trusts a dangling `origin/HEAD`. `git symbolic-ref` reads a symref and exits 0 without validating its target, so a pruned fetch that removed `refs/remotes/origin/<branch>` left the function naming a branch that exists nowhere while the `main`/`master` fallbacks never ran; the printed target is now probed with `git rev-parse --verify --quiet`, so a same-named local branch cannot re-admit a dead target, before the local chain takes over ([#399](https://github.com/LankfordAI/omp-ui/issues/399)).
...
6. `release-manifest` depends on successful Linux, macOS, and Windows jobs, and waits for `release-notes` to finish without gating on its result. It downloads exactly one AppImage, two DMGs, two ZIPs, and one Windows installer. It rejects duplicate or missing names, writes the sorted six-line `SHA256SUMS.txt`, and uploads it. It then calculates SHA-512 and size for all four macOS files, writes `latest-mac.yml`, and uploads the feed. Before publishing it warns in the run log when the draft's body is still empty.
