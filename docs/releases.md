...
- Fixed the Finish worktree dialog disabling **return this session** — and return/sync refusing — when the only uncommitted entry in the worktree checkout was omp-ui's own generated `.omp` project-config link, in repos that ignore `.omp/` with a trailing slash and on Windows, where the generated link is a junction git reports as an untracked directory (#417, #423).
- Fixed releasing a worktree session on Windows leaving the checkout directory standing: `removeWorktree` handed git a tree still holding omp-ui's generated `.omp` junction, and Git for Windows walks a junction as a directory rather than unlinking it, so the link itself could not be removed and the project's own config sat in the deletion's path. That link is now removed before either delete reaches the tree, and a checkout that owns a real `.omp` keeps it (#424).
...
- Added: while a question blocks the session, the desktop composer takes the place of the question's **Other** field — typing the answer and pressing Enter answers it there instead of queueing an invisible steering message behind the blocked tool, and the card drops its duplicate field for a hint line. Editor and input frames, and selects whose answer isn't constrained to the options, answer in one Enter; a multi-select frame still takes the *Other* sentinel and keeps its draft for the editor frame that follows; the compact sheet and the subagent view keep their own field (#421).
- Added **push**, **publish**, and a **pull request** link to the branch chip and the **Finish worktree** dialog: a merged merge-back now stays open on a *done* row that counts what the destination trails its own upstream and offers the push — the first push *publishes* with `git push -u` — then a pull-request link built from the remote's web origin, GitHub compare or GitLab new merge request, with credentials and ports dropped and no API call, `gh`, or token needed. Finishing still settles local state only, so sharing stays an explicit step of its own (#414).
