...
- Linux CI, release, and nightly jobs run on a dedicated omp-ui self-hosted pool where every job starts a fresh container from a pinned image under a single-job registration, and each such job first asserts that isolation; the shared `gfx1201` pool is no longer part of the release path ([#629](https://github.com/LankfordAI/omp-ui/issues/629)).
...
2. `release-linux` runs on the omp-ui self-hosted Linux pool (`omp-ui-linux`): a fresh container per job under a single-job registration (issue #629), asserted by the lane's first step. It installs dependencies, runs the repository typecheck and tests, runs the Electron-runtime `node-pty` smoke, stamps the version, builds the AppImage, and requires exactly one AppImage plus a `latest-linux.yml` that names it and contains SHA-512 metadata. It then uploads the AppImage and Linux metadata into the draft.
