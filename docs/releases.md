...
- Confirmations and backend failures now render as in-page **feedback dialogs** instead of renderer-blocking native window modals: stopping an agent, switching a session between terminal and native mode, and removing a project each open a store-driven DOM alertdialog a keyboard or an automated driver can answer. Only one dialog shows at a time — an error notice outranks a pending confirmation without dropping it, and dismissing the notice restores the confirmation (issue #373).
- The composer's **advisor control** and the **plan review** advisor row now report a gated instance honestly: under a dev/test spawn gate they name the pinned selector the process actually runs, tagged **dev/test**, instead of omp's configured model, and model and level editing is read-only while the gate is present — the on/off switch still drives the session's own saved choice, and nothing the gate shows enters a record (issue #372).
- Two environment seams gate a development run's spawns to a cheap model: **OMP_UI_TEST_MODEL** passes its `provider/model[:level]` selector to every session the instance spawns — fresh or resumed, terminal or native — as omp's `--model`, which reaches a resumed spawn the way a project's default-model pin never could, since a resumed session's model otherwise comes from the transcript. **OMP_UI_TEST_ADVISOR** pins the advisor model alone and leaves its on/off posture to the session record; with both unset, argv and overlays are byte-identical to before (issue #371).
- HTML plans rendered in **plan review** now sit on a soft gray canvas with charcoal ink instead of paper white with black — links and diagram text retuned to match — and code blocks step from the theme's sunken plane to its raised plane, so a dark theme's code reads as a raised card on the canvas rather than a black well, with every pair holding WCAG AA contrast on all 12 themes (issue #375).
- Every GitHub release page now ships with real **release notes**: a job resolves the previous tag, groups the range's commits by the issue or PR each subject references — PRs merged in the window list under the issue they cite, uncited commits land under *Other changes* — and writes **Highlights**, **What's Changed**, **New Contributors**, and a compare link into the draft while the macOS and Windows builds are in flight. The `## Unreleased` bullets here become the Highlights verbatim, and a notes failure never strands a finished release in draft (issue #377).
- Dead code an import-graph review surfaced is gone: the panel **resize handle** now steps by the shared keyboard-resize constant instead of duplicating its value as a literal, the `@omp-ui/core` and `@omp-ui/server` manifests no longer advertise export subpaths nothing imports, and the read-only visible-string audit script finally has a documented home in the development guide (issue #370).
