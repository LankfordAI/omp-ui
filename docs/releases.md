...
* HTML plan proposals are now validated by the application before a review can open ([#312](https://github.com/LankfordAI/omp-ui/issues/312) follow-up): main claims the proposal, renders the artifact in a hidden script-less Chromium verifier, and only a passed document reaches the review — a detectable source defect comes back to the agent as located, surgical-repair diagnostics in the proposal tool result instead of a broken plan on screen; an application failure says so without asking for a rewrite, and an inconclusive verification neither presents nor passes. Execute answers re-check the validated bytes, so a plan changed after validation invalidates the gate rather than starting implementation ([ADR-0022](adr/0022-prepared-plan-verification-in-the-renderer.md), amended).
* Fixed: HTML plan preparation corrupted code blocks whose text contained a `$` sequence — an authored `$'` (dollar-followed-by-quote, common in shell regexes) could splice the surrounding document into the block through JavaScript replacement-pattern semantics ([#412](https://github.com/LankfordAI/omp-ui/issues/412)). Prepared output is now composed from original source slices: untouched bytes, decoded dollar sequences, Unicode, and trailing newlines survive exactly, and plan text that merely looks like a renderer marker is ordinary source.
* Fixed: every plan document — the review dock, transcript card, and remote viewers alike — laid out sixteen pixels wider than its frame, an invisible horizontal scroll (mostly harmless, but real when a wide block reached the edge). The guardrail style sheet forced `width: 100%` on `body` while the browser's own 8px body margin still applied; the generated rule now keeps `width: auto`, which contains the margin by definition ([ADR-0022](adr/0022-prepared-plan-verification-in-the-renderer.md)).
