...
| Subagents inherit the session model | Spawns every subagent on the session's own model unless a per-agent choice (Agents pane, or `task.agentModelOverrides` at the project/global layer) says otherwise ([ADR-0031](adr/0031-subagent-model-selection-via-config-layers.md)). Off restores omp's own defaults: an agent's frontmatter model (scout and sonic bind `@smol`), else the session model. | Sessions launched afterwards, and running sessions whose per-agent choice is unset — session choices apply at the next subagent spawn. |
| Subagent concurrency | Caps how many subagents run at once (`task.maxConcurrency`); extra spawns queue instead of failing. Edited on the **omp** page: Global writes omp's global configuration, Project writes the focused project's `.omp/config.yml`, and clearing the Project value falls back to Global. | Sessions started afterwards. Existing omp processes keep the cap they started with. |
| Experiments lab (beta) | Turns on the whole Experiments surface: the Lab, the New experiment dialog, and the autoresearch bridge behind the session HUD chip (OMP's `/autoresearch` loop, [ADR-0030](adr/0030-experiments-read-autoresearch-from-two-sources.md)). Off hides those surfaces and leaves `/autoresearch` to OMP's own command. | The flag gates the bridge at spawn: it applies to sessions started afterwards. Sessions already running never get the bridge. |
| Skip the delete confirmation | Removes the warning shown before deleting a session, its whole lineage directory, and every plan implementation descendant. A delete that would remove a worktree checkout — the session's own or a descendant's — still asks. | Immediately. |
| Voice input | Adds a microphone button to every native-transcript composer ([ADR-0034](adr/0034-dictation-captured-in-renderer-transcribed-in-main.md)). Recordings are transcribed by the provider named by the dictation model below and inserted into the draft at the caret — never sent. Off hides the button everywhere, including remote web clients. | Immediately, in every composer. |
| Dictation model | Which speech-to-text model transcribes recordings. Rows come from the installed omp's STT catalog (`omp models --kind stt`), never a curated list ([ADR-0027](adr/0027-web-search-provider-list-discovered-from-omp.md)); each shows whether a stored provider key makes it callable. Auto resolves at call time to the first callable model. | The next recording. |
...
The **Subagent models** section lists one row per discovered agent, each with the layer badge of the layer that currently wins for that agent. A Global / Project scope chip chooses where the next edit lands ([ADR-0031](adr/0031-subagent-model-selection-via-config-layers.md)): Global edits rewrite the whole `task.agentModelOverrides` record through `omp config set` (it is replace-not-merge, like model roles); Project edits write a single entry into the focused project's `.omp/config.yml`, leaving hand-written siblings and comments untouched. A project file whose shape omp-ui cannot safely edit (a flow mapping, an anchor, a duplicate key) explains itself in place and disables the Project chip. Picking a value opens the model palette; its first two rows are always "session model" (`*`, inherit the base model) and "omp default" (clear the key at that layer). **Refresh agents** re-enumerates the roster from the installed omp. Global and Project choices apply to sessions started afterwards; session-scoped choices live in the Agents pane and apply at the next subagent spawn.

The **Subagent concurrency** section edits `task.maxConcurrency`, the cap on how many subagents a session runs at once — extra spawns queue instead of failing. Its Global / Project chips choose where the edit lands, exactly like the Subagent models section: Global commits through `omp config set`; Project writes this one key into the focused project's `.omp/config.yml`, leaving hand-written siblings and comments untouched, and **clear** removes the project override so the badge falls back to global/default. A project file whose shape omp-ui cannot safely edit explains itself in place and disables the Project chip. omp reads the setting when the session's process starts, so changes apply to sessions started afterwards.
