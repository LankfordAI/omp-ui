...
- [Getting started](getting-started.md) installs the supported Linux AppImage and the persistent host it carries, walks through the first project and session, and documents the `omp-ui` command and running the host as a login service.
...
- [Remote instances](remote-instances.md) joins other omp-ui hosts as remote instances: their projects and sessions in this sidebar, what crosses and what stays instance-local, reconnection, and credential storage.
...
- [Architecture](architecture.md) describes the host, desktop client, browser client, core, server, and renderer boundaries and their data flow.
