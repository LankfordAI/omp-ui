...
An AppImage launch edits omp-ui's own `process.env`: the runtime exports `APPDIR`, `APPIMAGE`, `ARGV0`, and `OWD`; electron-builder's AppRun prepends the mount to `PATH`, `XDG_DATA_DIRS`, `LD_LIBRARY_PATH`, and `GSETTINGS_SCHEMA_DIR`; and every electron-updater relaunch stacks another copy. Every child that runs the user's programs — the console drawer shell, every omp child through `ompChildEnv`, git, the login-shell key capture, and the project open targets (VS Code, Files, Terminal) — starts from core's [`withoutAppImageRuntime()`](../packages/core/src/appimage-env.ts), which drops those variables and undoes each generation's AppRun edits. It is recomputed per spawn so provider keys installed later still ride along (ADR-0010). On Linux, Electron's `shell.openExternal` and `shell.openPath` launch `xdg-open` from omp-ui's own environment and accept no replacement, so the VS Code and Files targets spawn `xdg-open` themselves ([`project-open.ts`](../packages/desktop/src/main/project-open.ts)). Browser handoffs — transcript links, provider sign-in, release notes, the exported transcript — stay on Electron's `shell`, whose launch carries the XDG activation token that lets an already-running browser raise its window; a browser one of them starts fresh still inherits the AppImage variables. PTYs rendered by xterm.js (terminal tabs, the console drawer, its handoff TUI) also advertise `COLORTERM=truecolor`. Electron's own variables (`NO_AT_BRIDGE`, `GDK_BACKEND`, `CHROME_DESKTOP`, …) pass through unchanged.
...
Desktop main owns the pane. `BrowserPaneHost` (`packages/desktop/src/main/browser-pane-host.ts`) keeps one offscreen `WebContents` per live rpc-ui tab in the app-wide `persist:browser-pane` partition, JPEG-encodes its `paint` frames and streams them on the lossy `browser-pane:frame` event, applies renderer input from `browser-pane:input`, and exposes the page to the agent through a loopback, token-pathed CDP bridge (`browser-pane-bridge.ts`) whose endpoint the generated `omp-ui-browser-pane.ts` extension delivers to omp as one hidden message. Two requests ride the same seam: `browser-pane:pick` is tab-routed for the element hand-back, while app-scoped `browser-pane:clear-data` is never proxied and clears this host's partition. The endpoint lives with the live rpc tab; the page is created on first open or first agent attach, survives a process restart, and is destroyed with hibernate, delete, and a switch to terminal mode. `@omp-ui/core` owns the pane wire contract and extension writer; `@omp-ui/server` owns the authenticated paired frame stream, sequence envelope and ACK credit. The relay does not re-encode JPEGs. Renderers hold no page handle: they receive frames and JSON state and send JSON input, and every pane channel is tab-routed. See [ADR-0029](adr/0029-browser-pane-offscreen-webcontents-and-loopback-cdp-bridge.md).
