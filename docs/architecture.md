...
flowchart LR
  ...
  ipc --> main[HostApplication<br/>@omp-ui/host, constructed by Electron main]
...
The browser path does not create a second application backend. `@omp-ui/server` accepts a `HostSurface`, dispatches each connection's requests and notifications to a `HostApplication.handlers(ctx)` table built for that connection, and mirrors backend events from `HostApplication.addSink()` to the sockets each event's scope names. Session ownership, the registry, child processes, and OMP updates live in `HostApplication` (`@omp-ui/host`), which release P still constructs inside the Electron main process. The same process is also the only client of any [remote instance](remote-instances.md) this app has joined: `RemoteInstanceManager` dials the other app's `@omp-ui/server`, and its projects and sessions arrive in the renderer as ordinary backend state.
...
| [`@omp-ui/desktop`](../packages/desktop/src/) | The Electron lifecycle, secure window and preload setup, the IPC bridge that binds `HostApplication.handlers(ipcCtx)` for the window, the desktop adapter (`window.ompDesktop`) for client effects, renderer and web builds, the Electron `safeStorage` cipher, window and shell integration, the OS notifier, the legacy-authority tripwire, and the app's own update state. | The authoritative application (`HostApplication`, `SessionManager`, remote-server lifecycle, `RemoteInstanceManager`) — those live in `@omp-ui/host`. A second transport-specific business interface. Both IPC and WebSocket use the core channel table. |
| [`@omp-ui/server`](../packages/server/src/index.ts) | Static delivery of the browser bundle, the credential-to-grant seam (`authenticate`), the hello-first protocol handshake, WebSocket request routing into per-connection tables, scoped event fan-out, binary framing for PTY and shell bytes, and the matching `connectInstanceClient` that desktop main uses to join another app's server. | Electron, the registry, sessions, OMP processes, token or password policy, or a standalone backend. It requires a `HostSurface` and an `authenticate` callback supplied by desktop main. |
| [`@omp-ui/plan-doc`](../packages/plan-doc/src/index.ts) | The browser-targeted HTML plan pipeline every surface shares: plan source parsing and composition, structural verification, document preparation, the Mermaid and code-highlight transforms, the layout probe and in-page preflight render, and the pure theme values the verifier page and the renderer both derive from. Electron-free and Node-free. | The renderer's React hooks (`usePreparedPlanDocument`, `useTheme`), or the verifier's queue, deadline, browser, and `unavailable` outcomes — those belong to the host's verifier. |
| [`@omp-ui/host`](../packages/host/src/index.ts) | The persistent host (issue #442): `HostApplication`, the authoritative composition root that the desktop constructs in release P and `omp-ui serve` constructs from release C; the headless Chrome-for-Testing plan verifier and its loopback static origin; and, built and tested but not reachable in P, the authority claim and children ledger, the migration journal with its relocation and credential-handoff steps, the host `KeyCipher` and its OS credential protectors, local control and the connection record, the CLI and platform supervisors, and the host updater. Its eslint rule forbids `electron`. | Electron, windows, client effects, or the renderer. No `bin`, service, or connection record is installed in release P. |
...
[`BACKEND_CHANNELS`](../packages/core/src/backend-channels.ts) declares each capability once. The channel name, arguments, result, generated `OmpBackend` client, and `ChannelTable` handler shape all derive from that declaration. [`HostApplication.handlers(ctx)`](../packages/host/src/host-application.ts) implements the request and notification sides once for both transports; the table is built per connection so role-gated channels can be omitted.
...
The Providers page's **Preferred provider** select writes omp's `providers.webSearchOrder` through the same `omp config set` boundary as every other global omp key (ADR-0025); omp-ui keeps no preference of its own. Its choices come from the installed binary: `readWebSearchProviders` in [`omp-settings.ts`](../packages/core/src/omp-settings.ts) probes `omp search --provider=<sentinel>` under an empty `HOME`, and the zero-import [`web-search-order.ts`](../packages/core/src/web-search-order.ts) subpath parses that rejection into provider ids and maps a stored order to and from the select. The pure half is bundled by the renderer; the spawn half stays main-process only, reached through the `web-search-providers:read` channel. omp validates neither the array nor its members, so the control is closed — free text is never offered — and a configured id omp's list lacks stays selectable, labelled as outside omp's list. An undiscoverable list degrades to `discovered: false` plus a visible note rather than a fabricated catalog ([ADR-0027](adr/0027-web-search-provider-list-discovered-from-omp.md)).
...
MCP configuration resolution stays in transport-agnostic core and returns only a redacted effective view: credentials, headers, auth and OAuth blocks, source errors, and URL secrets never reach a renderer. Resolution and writes are keyed on the working tree whose project-scope config decides, which for a worktree session is its checkout rather than the project root; the checkout carries a `.omp` symlink to the project's own directory so OMP resolves the project's config there and a write lands on the project's real file (issue #325). A live native session's connection truth comes from OMP's `mcp:connection-status` event bus through a third per-lineage generated extension. That extension reduces raw events to server names plus `auth` or `connection` failure kinds and publishes the snapshot over OMP's existing `setStatus` frame. `HostApplication` forwards the ordinary rpc frame, so Electron and remote-browser renderers derive the same transcript notice, Session HUD badge, and manager-row state without a new backend channel or session-file entry.
...
Desktop main owns whether remote access is enabled, its bind address and port, its token, its password hash, which credentials map to which client role, and server restart policy. `@omp-ui/server` owns HTTP delivery, the `/ws` upgrade, the protocol-2 hello (role and protocol-range verdict, close 4002 on mismatch), JSON request routing, and event fan-out by scope (broadcast, role, or one connection). It authenticates nothing itself: every presented credential — the minted token or the password-derived session credential — goes through desktop main's `authenticate` callback, which answers with a role grant or null. It has no access to registry or session implementation beyond the `HostSurface` interface. The browser reconnect path reloads and rehydrates instead of inventing a partial frame replay. See [Remote access](remote-access.md) for trust and network constraints.
