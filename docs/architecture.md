...
Desktop main owns the pane. `BrowserPaneHost` (`packages/desktop/src/main/browser-pane-host.ts`) keeps one offscreen `WebContents` per live rpc-ui tab in the app-wide `persist:browser-pane` partition, JPEG-encodes its `paint` frames and streams them on the lossy `browser-pane:frame` event, applies renderer input from `browser-pane:input`, and exposes the page to the agent through a loopback, token-pathed CDP bridge (`browser-pane-bridge.ts`) whose endpoint the generated `omp-ui-browser-pane.ts` extension delivers to omp as one hidden message. The endpoint lives with the live rpc tab; the page is created on first open or first agent attach, survives a process restart, and is destroyed with hibernate, delete, and a switch to terminal mode. `@omp-ui/core` owns the wire contract (`browser-pane.ts`: frame header codec, input and navigation unions, the hidden command) and the extension writer; `@omp-ui/server` carries frames on its structural binary rule and skips a client whose socket buffer is over the lossy threshold. Renderers hold no handle: they receive frames and JSON state and send JSON input, and every pane channel is tab-routed so a joined instance's pane streams through this app like its terminal does. See [ADR-0029](adr/0029-browser-pane-offscreen-webcontents-and-loopback-cdp-bridge.md).
