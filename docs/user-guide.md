...
The worktree chip on the Session HUD offers a **merge-back** when the session has a recorded base. It merges the worktree branch into that base inside the project checkout — fast-forward when history allows, otherwise a merge commit. Only committed work on the branch is merged; uncommitted changes in the worktree are not included. The action is disabled with a reason when it cannot run: the recorded base no longer resolves, no local branch matches it, the destination is not checked out in the project, the worktree branch was deleted, the branch is already merged into the destination, or a merge is already in progress. A conflicted merge stops and is left in the project checkout; resolve the files there with `git merge --continue`, or abort with `git merge --abort`. The chip's **open console** button opens the session's console drawer. The delete confirmation offers a **merge & delete** option that runs the same merge into the base first; if the merge stops on conflicts, the dialog stays open so you can delete without merging — the branch survives deletion either way. The composer's branch chip — which shows the worktree branch while a worktree session is focused — offers the same merge-back as the first row of its menu, with the same destination, commit count, and disabled states.

Deleting a worktree session first erases its lineage, then attempts to force-remove its checkout. If the checkout is removed, any uncommitted changes in it are lost. The branch and all commits survive in the original git repository. Checkout cleanup failure leaves the checkout on disk but does not stop deletion of the session record. If the checkout disappears outside omp-ui, resume fails instead of silently falling back to the project's main working tree.
