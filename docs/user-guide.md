...
The effective checkout controls the branch chip, `@` file picker, branch diff pane, console shell, and MCP manager scope. Resume, advisor restart, native or terminal mode restart, and Build or Plan switches all keep the same checkout because its path is stored on the session record.
...
- Working-tree scope uses the focused session's own working tree — its worktree checkout when it has one, otherwise its project root — or the project whose MCP action you opened. The manager's header names that directory, and a worktree session's header also names its branch and the project the write lands in.
