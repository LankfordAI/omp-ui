...
- Dictate instead of typing with the microphone button (issue #647). Click, speak, click again — or wait for the 60-second cap — and the transcript lands at the caret of your draft, never sent: edit it and press Enter yourself. Escape while recording discards the take. Enable the button under Settings → General → Voice input and choose the model under Dictation model; recordings are transcribed by that provider from the main process, so no key reaches the page.
