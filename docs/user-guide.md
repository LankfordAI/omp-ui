...
A new native session gets an **auto-title** from its first substantive prompt. Bare greetings and acknowledgements do not consume the title opportunity. omp-ui asks OMP's configured small model first, then derives a short title from the prompt if that model is unavailable or declines. When the session was seeded from an approved plan, the plan names it: the row reads the plan's title, never the implementation prompt that carried the plan. Prompt text beyond a generous bound is clipped before it reaches the model, so a large pasted body or plan spec cannot silently cost the session its model title.

**Re-titling** asks again what a session is about once its title has outgrown the work. Hover the Session HUD title for the retitle button, or run **Regenerate session title** in the command palette; either reads a digest of the session's transcript and replaces the title when the model finds a better name. It needs a running session — a dormant or hibernated one answers with a notice; resume it first, then re-title. A session whose transcript already reads well is left alone, and a title you typed by hand is never overwritten. Terminal tabs keep omp's own titles and offer no retitle action.
