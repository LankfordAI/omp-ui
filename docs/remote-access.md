...
The browser pane is hosted the same way. A browser connected to this app sees the pane live and can click, type, and navigate in it. Only the desktop window sizes the pane; other views scale its frames to fit their own. The server skips new frames once its socket buffer exceeds 64 KiB, so the desktop does not wait on a remote viewer. This does not remove images already queued in the network: a heavily throttled client can show stale frames and delayed terminal output on the shared connection. That limitation remains tracked in [#546](https://github.com/LankfordAI/omp-ui/issues/546); normal reconnection and frame dropping are not proof of bounded delivery latency.
...
- [Remote instances](remote-instances.md) joins another omp-ui app's projects and sessions into this app's sidebar over the same listener and credentials.
